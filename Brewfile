# Homebrew Bundle
# https://github.com/Homebrew/homebrew-bundle

# ------------------------------------------------------------------------------
# Taps
# ------------------------------------------------------------------------------
tap "homebrew/bundle"
tap "ngrok/ngrok"               # https://ngrok.com/

# ------------------------------------------------------------------------------
# CLI Tools
# ------------------------------------------------------------------------------
brew "awscli"                   # https://aws.amazon.com/cli/
brew "bat"                      # https://github.com/sharkdp/bat
brew "gh"                       # https://github.com/cli/cli
brew "git"                      # https://git-scm.com/
brew "helm"                     # https://helm.sh/
brew "hey"                      # https://github.com/rakyll/hey
brew "htop"                     # https://htop.dev/
brew "k3d"                      # https://k3d.io/
brew "k9s"                      # https://k9scli.io/
brew "kubernetes-cli"           # https://kubernetes.io/docs/reference/kubectl/
brew "lsd"                      # https://github.com/lsd-rs/lsd
brew "mas"                      # https://github.com/mas-cli/mas
brew "nvm"                      # https://github.com/nvm-sh/nvm
brew "poetry"                   # https://python-poetry.org/
brew "pre-commit"               # https://pre-commit.com/
brew "pyenv"                    # https://github.com/pyenv/pyenv
brew "pygments"                 # https://pygments.org/
brew "terminal-notifier"        # https://github.com/julienXX/terminal-notifier
brew "tldr"                     # https://tldr.sh/
brew "tree"                     # https://linux.die.net/man/1/tree
brew "unbound"                  # https://nlnetlabs.nl/projects/unbound/about/
brew "watch"                    # https://linux.die.net/man/1/watch
brew "wget"                     # https://www.gnu.org/software/wget/
brew "wifi-password"            # https://github.com/rauchg/wifi-password
brew "zsh"                      # https://www.zsh.org/
brew "zsh-completions"          # https://github.com/zsh-users/zsh-completions

# ------------------------------------------------------------------------------
# Cask Applications
# ------------------------------------------------------------------------------
cask "docker"                   # https://www.docker.com/
cask "font-fira-code"           # https://github.com/tonsky/FiraCode
cask "font-fira-code-nerd-font" # https://github.com/ryanoasis/nerd-fonts
cask "font-hasklug-nerd-font"   # https://github.com/ryanoasis/nerd-fonts
cask "google-chrome"            # https://www.google.com/chrome/
cask "iterm2"                   # https://iterm2.com/
cask "jetbrains-toolbox"        # https://www.jetbrains.com/toolbox-app/
cask "ngrok"                    # https://ngrok.com/
cask "postman"                  # https://www.postman.com/
cask "rectangle"                # https://rectangleapp.com/
cask "visual-studio-code"       # https://code.visualstudio.com/
cask "wireshark"                # https://www.wireshark.org/

